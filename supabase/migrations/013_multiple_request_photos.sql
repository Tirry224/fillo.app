-- Le formulaire client ne permettait d'envoyer qu'une seule photo par
-- demande (colonne photo_path unique sur client_requests et sales). On passe
-- à un tableau de photos (max 3, imposé côté application) pour que le
-- commerçant puisse voir toutes les images envoyées par le client.

alter table public.client_requests add column photos text[] not null default '{}';
update public.client_requests set photos = array[photo_path] where photo_path is not null;
alter table public.client_requests drop column photo_path;

alter table public.sales add column photos text[] not null default '{}';
update public.sales set photos = array[photo_path] where photo_path is not null;
alter table public.sales drop column photo_path;
