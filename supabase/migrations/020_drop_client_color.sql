-- clients.color n'est plus utilisé : la couleur affichée pour un client
-- (avatar) reprend désormais le statut de sa commande la plus pertinente,
-- calculé côté application (voir lib/utils/clientStatus.ts), au lieu d'être
-- une valeur fixe assignée à la création ('blue' dans tous les cas en
-- pratique). On retire donc la colonne, après avoir mis à jour la fonction
-- qui l'alimentait.
...
begin
  ...
  if existing_client_id is not null then
    ...
    insert into public.clients (
      shop_id, initials, name, phone, normalized_phone
    ) values (
      target_shop_id,
      customer_initials,
      trim(customer_name),
