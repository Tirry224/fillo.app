-- Un numéro de téléphone identifie un client de façon unique pour une boutique
-- donnée (contrainte unique (shop_id, normalized_phone) sur public.clients).
-- Depuis 006, une nouvelle demande avec un numéro déjà connu ne renomme plus
-- le client existant, mais elle réutilise silencieusement son identité même
-- si le nom saisi est différent : la commande d'une autre personne peut donc
-- se retrouver classée sous un client qui n'est pas le sien, sans avertir
-- personne. Ce numéro ne pouvant appartenir qu'à un seul client à la fois, la
-- demande est désormais rejetée dans ce cas : le formulaire public affiche un
-- message indiquant que ce numéro appartient déjà à un autre client (sans
-- révéler son nom, donnée d'un tiers, formulaire non authentifié).
